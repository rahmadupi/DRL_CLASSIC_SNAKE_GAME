...
def _infer_obs_type_from_shape(shape) -> Optional[str]:
    ...
    * ``(4, H, W)``  → ``"spatiotemporal"``        (honest layout — current default;
                                                     ``spatiotemporal_legacy`` produces
                                                     the same shape and is disambiguated
                                                     by the filename-regex fallback)
    * ``(12,)``      → ``"12bit"``
    * else           → ``None`` (unknown)

    Note: the previous v2 layout used an 8-channel ``(8, H, W)`` tensor.
    Saved models from that era will no longer load — the honest layout
    is the only supported training obs. Old filenames still resolve
    correctly via the regex fallback in :func:`_infer_obs_type_from_filename`.
    """
...
class input_controller:
    ...
    def process_ai_input(self, obs=None) -> int:
        ...
        Args:
            obs: Current observation from the env. Shape/type must
                 ...
                 (4×20×20 for "spatiotemporal" or "spatiotemporal_legacy",
                 12-dim for "12bit"). If None, falls back to a random
                 action.
    ...
    def load_model(self, model_path: str) -> Dict[str, Any]:
        ...
        purely so old filenames still resolve correctly via the
        obs_type-detection regex; both obs types produce the same
        4-channel honest layout at runtime.
        ...
        if obs_type is None:
            raise RuntimeError(
                ...
                f"{model_path!r}. Expected spatiotemporal (4,H,W) or "
                f"12bit (12,)."
            )
